...
bool Preprocess(const path& in_file, const path& out_file, const vector<path>& include_directories) {
    ...
    auto process = [&](auto& self, istream& in, ostream& out, const path& current_file) -> bool {
        string line;
        int line_number = 0;
        ...
        while (getline(in, line)) {
            ...
            if (!self(self, inc_stream, out, found_path)) {
                return false;
            }
        }
    ...
    return process(process, input, output, in_file);
}
